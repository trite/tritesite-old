...
let literal =
  trueBool <|> (lazy(falseBool)) <|> (lazy(quotedString)) <|> (lazy(number));
        ...
        _ =>
          ...
          >>= (
            iden =>
              ...
              >>= (
                _ =>
                  listR(Lazy.force(expr))
                  >>= (
                    value =>
                      whitespace
                      >>= (
                        _ => closeParen <$> (_ => ProcedureCall(iden, value))
                      )
                  )
              )
          )
    ...
    _ =>
      ...
      >>= (
        _ =>
          listR(Lazy.force(expr))
          >>= (
            value => whitespace >>= (_ => closeParen <$> (_ => List(value)))
          )
      )
  );
